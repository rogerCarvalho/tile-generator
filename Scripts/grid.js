$.fn.moduleGrid = function(){
	...
	Tile.prototype.findMyPlace = function(){
	    var type = this.DT.attr('class');
	    var forma = geralTranslateType(type);

	    console.log(type);

		for (var i in ocupVal) {
		    var origem = ocupVal[i]
                ,checkList = []
		        ;

		    if (forma.w - forma.h == 0) {
		        for (var i = 0; i < forma.w; i++) {
		            var coluna = origem + i * 10;
		            var linha = coluna + 1;

		            for (var j = 0; j < forma.h; j++) {
		                linha = coluna + j + 1;
		                checkList.push(linha);
		            }

		            checkList.push(coluna);
		        }
		    } else {
		        var multiplicador
                    , contador
		        ...
		        for (var i = 0; i < contador; i++) {
		            var item = origem + i * multiplicador;
		            checkList.push(item);
		        }
		    }

		    if (this.placing(checkList, origem)) return;
		}
	}

	Tile.prototype.placing = function (checkList, item) {
	...
	var cont = 0;
	var ocupVal;
	var leTipo = ['doisdois', 'doisum', 'vertical', 'trestres'];
